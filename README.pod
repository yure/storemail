...
Mails sent are added to send queue so you have to run bin/mail_queue.pl to send mails on queue. By default it checks for new mail on queue every 10sec.

=head3 Gmail fetch

perl bin/get_gmail.pl [--sleep 10]  > get_mail_log.txt

You have to set 'domain' and 'gmail' setting in config. 

=head4 Initial import

perl get_gmail.pl -i 1 > get_mail_log.txt
